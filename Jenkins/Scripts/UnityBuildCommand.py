...
Config.save_config(Config.KEY.UNITY_BUILD_LOG, Logfile)
sys.stdout = sys.__stdout__

# Get unity path
unityPath = Config.read_config("UNITY_PATH")  # this one already have two quote around

pipeline = Config.read_config(Config.KEY.PIPELINE)

BuildMethod = {
    'release': Config.KEY.PRODUCTION_BUILD_METHOD_NAME,
    'dev': Config.KEY.DEVELOP_BUILD_METHOD_NAME,
    'internal': Config.KEY.INTERNAL_BUILD_METHOD_NAME
}.get(pipeline, Config.KEY.DEFAULT_BUILD_METHOD_NAME)

value = Config.read_config(BuildMethod)
if value is not None:
    UnityBuildMethodName = value

value = Config.read_config(Config.KEY.BUILD_TARGET)
if value is not None:
    BuildTarget = value

value = Config.read_config(Config.KEY.UNITY_BUILD_PARAMS)
if value is not None:
    Params = value
...
command = f'{BuildScript} -unityProject "{ProjectFolder}" -buildTarget {BuildTarget} -unityPath "{unityPath}" -logFile {LogFileName} -method {UnityBuildMethodName} -addGitLog '
